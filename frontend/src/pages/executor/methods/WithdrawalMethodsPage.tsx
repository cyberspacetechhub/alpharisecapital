...
export default function WithdrawalMethodsPage() {
  ...
  const { data, isLoading } = useQuery<WithdrawalMethod[]>({
    ...
    queryFn: () => withdrawalMethodApi.getAll().then((r) => r.data.data),
  });
  ...
  const openEdit = (m: WithdrawalMethod) => {
    const details = Object.entries(m.details).map(([key, value]) => ({
      key,
      value: value === "" ? "" : value,
      isInput: value === "",
    }));
  ...
  const buildPayload = (data: MethodForm) => ({
    ...
    details: Object.fromEntries(
      data.details.filter((d) => d.key.trim()).map((d) => [d.key.trim(), d.isInput ? "" : d.value.trim()])
    ),
    isActive: data.isActive,
  });

  const createMutation = useMutation({
    mutationFn: (data: MethodForm) => withdrawalMethodApi.create(buildPayload(data)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["withdrawal-methods-all"] }); setModalMode(null); },
    onError: (e: any) => setServerError(e?.response?.data?.message ?? "Failed to create method"),
  });

  const updateMutation = useMutation({
    mutationFn: (data: MethodForm) => withdrawalMethodApi.update(editing!._id, buildPayload(data)),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["withdrawal-methods-all"] }); setModalMode(null); },
    onError: (e: any) => setServerError(e?.response?.data?.message ?? "Failed to update method"),
  });

  const toggleMutation = useMutation({
    mutationFn: (id: string) => withdrawalMethodApi.toggle(id),
    onSuccess: () => qc.invalidateQueries({ queryKey: ["withdrawal-methods-all"] }),
  });

  const deleteMutation = useMutation({
    mutationFn: (id: string) => withdrawalMethodApi.remove(id),
    onSuccess: () => { qc.invalidateQueries({ queryKey: ["withdrawal-methods-all"] }); setDeleteTarget(null); },
  });
  ...
  return (
    <div className="max-w-4xl mx-auto space-y-5">
      ...
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-lg font-bold text-gray-800">Withdrawal Methods</h1>
          <p className="text-xs text-gray-400 mt-0.5">Manage payment systems available to traders for withdrawals.</p>
        </div>
        <button
          onClick={openCreate}
      ...
      {isLoading ? (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 px-6 py-16 text-center">
          <p className="text-sm text-gray-400">No withdrawal methods yet. Add one to get started.</p>
        </div>
      ) : (
        <div className="space-y-3">
          {methods.map((m) => (
                  ...
                  <div>
                    ...
                    <div className="mt-2 flex flex-wrap gap-x-4 gap-y-1">
                      {Object.entries(m.details).map(([k, v]) => (
                        <div key={k} className="text-xs text-gray-500 flex items-center gap-1">
                          ...
                          {v === "" ? (
                            ...
                            <span className="font-mono break-all text-gray-700">{v}</span>
                          )}
                        </div>
                      ))}
                    </div>
          ...
          <form onSubmit={handleSubmit(onSubmit)} noValidate>
            <div className="space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Min Amount ($) <span className="text-red-400">*</span></label>
                  <input
                    type="number"
                    {...register("minAmount", { required: "Min amount is required", min: { value: 1, message: "Must be >= 1" } })}
                    className={inputClass}
                  />
                  {errors.minAmount && <p className="text-xs text-red-500 mt-1">{errors.minAmount.message}</p>}
                </div>
                <div>
                  <label className="block text-xs font-semibold text-gray-600 mb-1.5">Max Amount ($) <span className="text-red-400">*</span></label>
                  <input
                    type="number"
                    {...register("maxAmount", { required: "Max amount is required", min: { value: 1, message: "Must be >= 1" } })}
                    className={inputClass}
                  />
                  {errors.maxAmount && <p className="text-xs text-red-500 mt-1">{errors.maxAmount.message}</p>}
                </div>
              </div>

              <div>
                <div className="flex items-center justify-between mb-1.5">
                  <label className="block text-xs font-semibold text-gray-600">Method details & fields <span className="text-red-400">*</span></label>
                  <button
                    type="button"
                ...
                <p className="text-xs text-gray-400 mb-2">Configure read-only instructions or inputs for the Trader (e.g. Wallet Address).</p>
                <div className="space-y-3">
                  {fields.map((field, i) => (
                    <div key={field.id} className="space-y-2 p-3 bg-gray-50 border border-gray-150 rounded-xl">
                      <div className="flex gap-2">
                        ...
                        {watch(`details.${i}.isInput` as any) ? (
                          <div className="flex-1 bg-gray-100 border border-gray-200 text-gray-400 text-xs px-4 py-2.5 rounded-xl flex items-center justify-center font-medium italic">
                      ...
                      <div className="flex items-center justify-between">
                        <label className="flex items-center gap-1.5 text-xs text-gray-500 font-medium select-none cursor-pointer">
                          <input
                            type="checkbox"
      ...
      {/* Delete confirm */}
      {deleteTarget && (
        <Modal title="Delete Withdrawal Method" onClose={() => setDeleteTarget(null)}>
